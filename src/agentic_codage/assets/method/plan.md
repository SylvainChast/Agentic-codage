# Produce an executable bounded plan

For Feature/Product reference the selected design and stories; for Express a concise plan with the
reproduction/desired behavior and verification is sufficient. With UI in Express, reference the
selected design system. Resolve material unknowns before dispatching implementation.
List work items with exact writable paths, expected behavior, dependencies, interface references
and runnable verification. Parallelize only independent scopes. Include integration/review work,
the likely call count and recorded budget. Do not split trivial edits into many paid calls.
Map each acceptance criterion to implementation and a check or explicit manual inspection. State
what remains a judgment instead of disguising it as an automated test. Save the plan without coding.
...
Use `framework method prompt plan --task TASK` with this step's name. Inspect the returned references
...
including failed framing and review, with `run record --stage plan` and the appropriate purpose
...
`framework method record TASK --kind plan --file docs/PATH.md --author ACTOR --input A-INPUT`.
