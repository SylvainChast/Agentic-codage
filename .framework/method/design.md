# Design one feature's solution

Reference the selected stories, applicable architecture and, when UI is enabled, the design system.
Inspect repository conventions. Explain how the user journey maps to components, data and interfaces;
cover the consequential empty/error/loading and failure cases. Name changed contracts, migrations,
compatibility constraints and the tests that will show correctness. State open decisions before
coding. Keep this a local design delta; do not reproduce the whole architecture or write all the
future implementation into the plan. A short diagram is useful only if it clarifies the decision.
...
Use `framework method prompt design --task TASK` with this step's name. Inspect the returned references
...
including failed framing and review, with `run record --stage design` and the appropriate purpose
...
`framework method record TASK --kind design --file docs/PATH.md --author ACTOR --input A-INPUT`.
