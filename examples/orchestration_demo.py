...
from agentic_codage.costs import report, task_report
from agentic_codage.interfaces import import_interface
...
if role == 'orchestrator' and payload.get('phase') == 'checkpoint':
    ...
    result = dict(summary='SIMULATION: deterministic file edit', acknowledged_interfaces=payload['interface_references'], change_requests=[])
elif role == 'reviewer':
...
def main():
    with tempfile.TemporaryDirectory(prefix='agentic-offline-') as directory:
        ...
        import_interface(store, dict(id='I-value-v1', created_at=now(), name='value', version=1,
            paths=['src/value.py'], specification='value is the integer 42, not a formatted string.',
            checks=['actual-value-test'], supersedes=None))
        task = create_task(store, title='Offline example', owner='demo-controller', scope=['src/value.py'],
            ...
            resources=[], decisions=[], depends_on=[], interfaces=['I-value-v1'])
        git('add', '.')
        ...
        print(canonical(dict(simulated=True, paid_calls=0, state='integrated',
              ...
              delivery=task_report(store, task['id'])['delivery'],
